...
AILex is a living dictionary. The public site and the `main` branch are expected to change as terminology, evidence, definitions, and editorial judgments evolve.
...
The Living Dictionary is the current public version of AILex.

- Source: `main`
- Public site: the current canonical AILex site
- Mutable: yes
...
The Living Dictionary is the version readers should normally use when they want the most current AILex entry.
...
For example, **AILex 2026** represents the dictionary's editorial state through its declared 2026 cutoff. The release itself may be created shortly after the cutoff while retaining the 2026 edition name.
...
The repository commit is the archival unit. AILex does not copy the entire website into a new `/2026/`, `/2027/`, or similar directory for each edition.
...
4. An immutable Git tag using the convention `ailex-YYYY`.
5. A GitHub Release titled `AILex YYYY` pointing to that tag.
...
Tags for published editions must not be moved to later commits.
...
An edition can be released after the cutoff. For example, a snapshot finalized in early January 2027 may still be **AILex 2026** when it represents the reviewed dictionary state through the 2026 cutoff.
...
A published annual edition remains immutable so it continues to represent what AILex contained at that historical point. If an important correction affects a released edition:
...
> AILex. *AILex 2026: Dictionary of the AI Era*. Annual edition, 2026. GitHub release/tag `ailex-2026`.
...
The first planned annual snapshot is **AILex 2026**. Until that snapshot is formally released, AILex remains the Living Dictionary and no historical annual edition should be presented as already published.
